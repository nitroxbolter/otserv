...
int32_t Player::getArmor() const
{
	...
	static const slots_t armorSlots[] = {CONST_SLOT_HEAD, CONST_SLOT_NECKLACE, CONST_SLOT_ARMOR, CONST_SLOT_LEGS, CONST_SLOT_FEET, CONST_SLOT_RING, CONST_SLOT_DECK, CONST_SLOT_BADGE, CONST_SLOT_SHIP, CONST_SLOT_WINGS, CONST_SLOT_AURA };
...
ReturnValue Player::queryAdd(int32_t index, const Thing& thing, uint32_t count, uint32_t flags, Creature*) const
{
	...
	if ((slotPosition & SLOTP_HEAD) || (slotPosition & SLOTP_NECKLACE) ||
	        ...
	        (slotPosition & SLOTP_RING) || (slotPosition & SLOTP_DECKBAD) || (slotPosition & SLOTP_SHIP) || (slotPosition & SLOTP_WINGAUR)) {
	...
	switch (index) {
		...
		case CONST_SLOT_WINGS: {
			if (slotPosition & SLOTP_WINGAUR) {
				ret = RETURNVALUE_NOERROR;
			}
			break;
		}
