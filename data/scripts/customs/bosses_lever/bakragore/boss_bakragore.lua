    ...
    if bossesKilled > 0 and bossesKilled < 2 then
        if bossesKilled == 1 then
            if creature:getSex() == PLAYERSEX_FEMALE then
                ...
                creature:addOutfit(922)
                creature:addOutfitAddon(922, 1)
                creature:addOutfitAddon(922, 2)
            elseif creature:getSex() == PLAYERSEX_MALE then
                creature:sendTextMessage(MESSAGE_INFO_DESCR, "You received the Decaying Defender full outfit.")
                creature:addOutfit(921)
                creature:addOutfitAddon(921, 1)
                creature:addOutfitAddon(921, 2)
            end
        end
    end
    return true
...
local loginSpawnChance = CreatureEvent("bakragoreOutfitandDeath")

function loginSpawnChance.onLogin(player)
	player:registerEvent("bakragoreOutfit")
	return true
end
