local config = {
    actionId = 30032, 
    bossName = "gnomevil",
    bossPosition = Position(646, 545, 10), 
    bossArea = {
        fromPos = Position(636, 535, 10), 
        toPos = Position(656, 556, 10), 
        entrancePos = Position(646, 553, 10), 
        exitPosition = Position(614, 545, 10) 
    },
    ...
    createTeleportPos = Position(646, 537, 10), 
    teleportToPosition = Position(614, 545, 10), 
    teleportRemoveSeconds = 60, 
    kickParticipantAfterSeconds = 60 * 20, 
    leverIds = {1945, 1946} 
}
...
local plaguerootAction = Action()

function plaguerootAction.onUse(player, item, fromPos, target, toPos, isHotkey)
    local participants = {}
    ...
    boss:registerEvent("plaguerootSystemDeath")
...
plaguerootAction:aid(config.actionId)
plaguerootAction:register()

local creatureEvent = CreatureEvent("plaguerootSystemDeath")
