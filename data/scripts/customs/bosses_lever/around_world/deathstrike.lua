local config = {
    ...
    bossPosition = Position(485, 499, 9), 
    bossArea = {
        fromPos = Position(475, 489, 9), 
        toPos = Position(494, 508, 9), 
        entrancePos = Position(485, 505, 9), 
        exitPosition = Position(501, 488, 6) 
    },
    ...
    createTeleportPos = Position(477, 499, 9), 
    teleportToPosition = Position(501, 488, 6), 
...
local theWelterAction = Action()

function theWelterAction.onUse(player, item, fromPos, target, toPos, isHotkey)
    local participants = {}
    ...
    boss:registerEvent("theWelterSystemDeath")
...
theWelterAction:aid(config.actionId)
theWelterAction:register()

local creatureEvent = CreatureEvent("theWelterSystemDeath")
